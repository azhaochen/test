...
var getLoginInfo = function(code, callback){
	if(!empty(req.cookies.openid) && !empty(req.cookies.access_token)){
		...
		loginInfo.nick = decodeURIComponent(req.cookies.nick);
		...
		api.getAccessToken(code,function(err,result){
			...
			*/
			api.getUser(result.data.openid, function(err,userinfo){
				...
				res.cookie('openid', loginInfo.openid, {path:'/',expires:new Date(Date.now()+result.data.expires_in-300)});
				res.cookie('access_token', loginInfo.access_token, {path:'/',expires:new Date(Date.now()+result.data.expires_in-300)});
				res.cookie('refresh_token', loginInfo.refresh_token, {path:'/',expires:new Date(Date.now()+29*86400)});
				res.cookie('head', loginInfo.head, {path:'/',expires:new Date(Date.now()+result.data.expires_in-300)});
				res.cookie('nick', encodeURIComponent(loginInfo.nick), {path:'/',expires:new Date(Date.now()+result.data.expires_in-300)});
...
module.exports = function (request, response) {
	...
	if(!empty(req.query.code)){
		loginInfo = getLoginInfo(req.query.code,StartApp);	//weixin登录跳转回来
	}else{
		var isLogin = 0;
		if(!empty(req.cookies.openid) && !empty(req.cookies.access_token)){
			isLogin = 1;
		...
		if(isLogin==0){
			...
			loginInfo = getLoginInfo('',StartApp);				//已登录，从cookie里面取登录信息
		}
	}
};
...
var getLoginInfo = function(code, callback){
	if(!empty(req.cookies.openid) && !empty(req.cookies.access_token)){
		...
		loginInfo.refresh_token = req.cookies.refresh_token;
		loginInfo.head = req.cookies.head;
		loginInfo.nick = req.cookies.nick;	//express自己会进行urldecode编码和解码。decodeURIComponent(req.cookies.nick);
		typeof callback=='function' && callback();
	}
	if(!empty(code)){
		//查询code对应的access_token及用户信息，并保存在cookie里面
